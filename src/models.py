...
#     def __init__(self, vocab_size, d_model, max_len=5000, dropout=0.2):
...
class ALGNet(nn.Module):
    def __init__(self, vocab_size, ehr_adj, ddi_adj, emb_dim=64, device=torch.device('cpu:0'), ddi_in_memory=True):
        ...
        self.pos_emb = nn.ModuleList(
            [PositionalEncodingLayer(emb_dim) for i in range(K-1)])

        self.end_linear = nn.Linear(emb_dim, emb_dim * 2)
        
        self.attention = nn.ModuleList([
            MultiheadAttentionEncoder(emb_dim, emb_dim, num_heads) for _ in range(K-1)
        ])
        ...
        self.ehr_gcn = LGC(voc_size=vocab_size[2], emb_dim=emb_dim, adj=ehr_adj, device=device)
        self.ddi_gcn = LGC(voc_size=vocab_size[2], emb_dim=emb_dim, adj=ddi_adj, device=device)
        self.inter = nn.Parameter(torch.FloatTensor(1))
    ...
    def forward(self, x):
        ...
        for adm in x:
            ...
            i2 = mean_embedding(self.embeddings[1](torch.LongTensor(adm[1]).unsqueeze(dim=0).to(self.device)))
            i1_seq.append(i1)
            i2_seq.append(i2)
        i1_seq = torch.cat(i1_seq, dim=1) #(1,seq,dim)
        i2_seq = torch.cat(i2_seq, dim=1) #(1,seq,dim)
        p1_seq = self.pos_emb[0](i1_seq)
        p2_seq = self.pos_emb[1](i2_seq)
        a1, h1 = self.attention[0](p1_seq)
        
        a2, h2 = self.attention[0](p2_seq)
        # Weight Sum
        # o1, h1 = self.encoders[0](
        #     i1_seq
        # )
        
        # o2, h1 = self.encoders[1](
        #     i2_seq
        # )
        ...
        patient_representations = torch.cat([a1, a2], dim=-1).squeeze(dim=0) # (seq, dim*4)
        queries = self.query(patient_representations) # (seq, dim)
